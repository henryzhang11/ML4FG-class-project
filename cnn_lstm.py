...
def train_model(model, dataset, epochs=100, patience=10, verbose = True):
    ...
    optimizer = torch.optim.RMSprop(model.parameters(), lr = 1e-3, eps = 1e-7, centered = False)
...
if __name__ == '__main__':
 ...
 #full path to the directory holding all of the data 
    ...
    for p,protein in enumerate(os.listdir(dataDir)):
        ...
        cnnlstm = CNN_LSTM(number_of_clip_experiments)
        train_model(cnnlstm, trainingData, epochs=30, patience=5, verbose = True)

        #cnnfc_1 = CNN_FC(number_of_clip_experiments)
        #train_model(cnnfc_1, trainingData, epochs = 100, patience = 5, verbose = True)

        #cnnfc_2 = CNN_FC(number_of_clip_experiments)
        #train_model(cnnfc_2, trainingData, epochs = 100, patience = 5, verbose = True)

        # calculate roc curve
        optimizer = torch.optim.RMSprop(cnnlstm.parameters(), lr = 1e-3, eps = 1e-7, centered = False)
        number_of_clip_experiments = testingData.loadData(p)
        test_dataloader = torch.utils.data.DataLoader(testingData, batch_size=100)
        _, _, true_y, predicted_y = run_one_epoch(False, test_dataloader, cnnlstm, optimizer)
